//=====Базовые значения=====//
let slots_per_reel = 8; //кол-во ячеек в барабане

let reels = 3; //кол-во барабанов

let slotAngle = 360 / slots_per_reel; //шаг поворота
...
let reel_radius = getReelRadius(slots_per_reel);

//Запоминаем углы поворота барабанов
let rotateAngle = [];

//Текущий выпавший слот
let currentSlot = [8, 8, 8]
...
function getReelSet2(count) {
	let ReelSet = $.ajax({
		url: 'http://pazhurdev.herokuapp.com/api/default_reel_sets/' + count + '/',
		method: 'GET',
		async: false,
		contentType: 'application/json'
		// success: function () {
